...
$(document).ready(function () {
  ...
  if ($(".btn-process").length) {
    $(".btn-process").on("click", function (e) {
      $(".overlay__loader").css({ display: "block" });
      var url = $(this).attr("href");
      e.preventDefault();

      $.ajax({ type: "GET", url }).done(function (data) {
        $(".overlay__loader").css({ display: "none" });
      });
    });
  }
});
